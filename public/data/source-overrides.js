/* Application-ready evidence overrides for browser-rendered portfolio/CV pages. */
(function applyBrowserEvidenceOverrides() {
  ...
  if (Array.isArray(D.stats)) {
    ...
    const graySwanStat = {
      ...
      detail:
        "106 platform-recorded challenge breaks in the archived 24 July 2026 snapshot; the public Arena profile also reported 24 global unique breaks, 1,050 points, and 242 submissions. Dated evidence: mariomarcolongo.com/evidence/gray-swan-profile-2026-07-24.html",
    };
    const existing = D.stats.findIndex(
      (stat) => stat && stat.label === "Gray Swan Proving Ground",
    );
    if (existing >= 0) D.stats[existing] = graySwanStat;
    else D.stats.unshift(graySwanStat);
  }

  if (Array.isArray(D.projects)) {
    const entropy = D.projects.find(
  ...
  if (Array.isArray(D.experience)) {
    const graySwan = D.experience.find(
      (experience) =>
    ...
    const entropyExperience = D.experience.find(
      (experience) =>
        experience &&
  ...
  function relabelPortfolioLinks() {
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => {
      relabelPortfolioLinks();
      window.setTimeout(relabelPortfolioLinks, 0);
    });
  } else {
    relabelPortfolioLinks();
    window.setTimeout(relabelPortfolioLinks, 0);
